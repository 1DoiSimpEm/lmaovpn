...
androidComponents {
    onVariants { variant ->
        val variantDirName = variant.name.replace(Regex("([a-z])([A-Z])"), "$1/$2").lowercase()
        val genBaseDir = layout.buildDirectory.dir("generated/source/ovpn3swig/${variantDirName}")
        val genSrcDir = genBaseDir.map { it.dir("net/openvpn/ovpn3") }

        val task = tasks.register<Exec>("generateOpenVPN3Swig${variant.name}") {
            doFirst {
                genSrcDir.get().asFile.mkdirs()
            }
            // Use lazy evaluation for command line arguments
            argumentProviders.add {
                listOf("-outdir", genSrcDir.get().asFile.absolutePath, "-outcurrentdir", "-c++", "-java", "-package", "net.openvpn.ovpn3",
                    "-Isrc/main/cpp/openvpn3/client", "-Isrc/main/cpp/openvpn3/",
                    "-DOPENVPN_PLATFORM_ANDROID",
                    "-o", "${genSrcDir.get().asFile.absolutePath}/ovpncli_wrap.cxx", "-oh", "${genSrcDir.get().asFile.absolutePath}/ovpncli_wrap.h",
        ...
        // Add the generated source directory to the variant's java sources
        android.sourceSets.getByName(variant.name).java.srcDir(task.map { genBaseDir.get().asFile })
    }
}
