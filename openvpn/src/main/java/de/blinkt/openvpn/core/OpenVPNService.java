...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        // Removed action buttons (pause/resume/disconnect) - notification is info-only
        // Clicking notification will open the app via getGraphPendingIntent()
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && DISCONNECT_VPN.equals(intent.getAction())) {
            // User requested disconnect - stop VPN and unschedule keepVPNAlive
            keepVPNAlive.unscheduleKeepVPNAliveJobService(this);
    ...
    {
        Log.i("OpenVPNService", "=== fetchVPNProfile() CALLED ===");
        Log.d("OpenVPNService", "Intent: " + (intent != null ? "not null" : "null"));
        if (intent != null) {
            Log.d("OpenVPNService", "Intent has EXTRA_PROFILEUUID: " + intent.hasExtra(EXTRA_PROFILEUUID));
            if (intent.hasExtra(EXTRA_PROFILEUUID)) {
                String profileUUID = intent.getStringExtra(EXTRA_PROFILEUUID);
                Log.i("OpenVPNService", "Profile UUID from intent: " + profileUUID);
            }
        }
        ...
        if (intent != null && intent.hasExtra(EXTRA_PROFILEUUID)) {
            ...
            if (startReason == null)
                startReason = "(unknown)";
            Log.i("OpenVPNService", "Looking up profile: UUID=" + profileUUID + ", version=" + profileVersion);
            // Try for 10s to get current version of the profile
            vpnProfile = ProfileManager.get(this, profileUUID, profileVersion, 100);
            if (vpnProfile != null) {
                Log.i("OpenVPNService", "=== PROFILE FOUND === name=" + vpnProfile.getName() + ", UUID=" + vpnProfile.getUUIDString());
            } else {
                Log.e("OpenVPNService", "=== PROFILE NOT FOUND === UUID: " + profileUUID);
            }
            ...
            Log.d("OpenVPNService", "Intent is null or has no EXTRA_PROFILEUUID, using last connected profile");
            ...
            if (vpnProfile == null) {
                ...
                if (vpnProfile == null) {
                    Log.e("OpenVPNService", "=== NO PROFILE FOUND === Returning null!");
                    return null;
                }
            }
        ...
        Log.i("OpenVPNService", "=== fetchVPNProfile() RESULT === name=" + name + ", reason=" + startReason);
    ...
    private void startOpenVPN(Intent intent, int startId) {
        Log.i("OpenVPNService", "=== startOpenVPN() CALLED ===");
        VpnProfile vp = fetchVPNProfile(intent);
        if (vp == null) {
            Log.e("OpenVPNService", "=== PROFILE IS NULL === Stopping service!");
            stopSelf(startId);
            return;
        }
        Log.i("OpenVPNService", "=== PROFILE LOADED === name=" + vp.getName() + ", checking VPN permission...");

        if (!checkVPNPermission(vp)) {
            Log.w("OpenVPNService", "=== VPN PERMISSION NOT GRANTED ===");
            return;
        }
        Log.i("OpenVPNService", "=== VPN PERMISSION GRANTED === Starting OpenVPN process...");
        ...
        if (useOpenVPN3) {
            ...
            Log.d("OpenVPNService", "Creating OpenVPNThread with argv: " + java.util.Arrays.toString(argv));
            Log.d("OpenVPNService", "Native library dir: " + nativeLibraryDirectory);
            Log.d("OpenVPNService", "Tmp dir: " + tmpDir);
            processThread = new OpenVPNThread(this, argv, nativeLibraryDirectory, tmpDir);
        }

        synchronized (mProcessLock) {
            mProcessThread = new Thread(processThread, "OpenVPNProcessThread");
            Log.d("OpenVPNService", "Starting OpenVPN process thread");
            mProcessThread.start();
            Log.d("OpenVPNService", "OpenVPN process thread started");
        }

        if (!useOpenVPN3) {
            try {
                Log.d("OpenVPNService", "Waiting for OpenVPN process stdin to be ready...");
                OutputStream stdin = ((OpenVPNThread) processThread).getOpenVPNStdin();
                Log.d("OpenVPNService", "OpenVPN process stdin ready, writing config file");
                
                // Log config file content for debugging
                String configContent = mProfile.getConfigFile(this, false);
                Log.d("OpenVPNService", "Generated config file (first 500 chars): " + 
                    (configContent.length() > 500 ? configContent.substring(0, 500) + "..." : configContent));
                
                mProfile.writeConfigFileOutput(this, stdin);
                Log.d("OpenVPNService", "Config file written successfully, closing stdin");
            } catch (IOException | ExecutionException | InterruptedException | java.util.concurrent.CancellationException e) {
                String errorMsg = "Error generating config file: " + e.getMessage();
                VpnStatus.logException("Error generating config file", e);
                Log.e("OpenVPNService", errorMsg, e);
                endVpnService();
                return;
            }
        }
