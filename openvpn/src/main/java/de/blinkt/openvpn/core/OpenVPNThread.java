...
public class OpenVPNThread implements Runnable {
    ...
    public void run() {
        try {
            Log.i(TAG, "Starting openvpn with args: " + java.util.Arrays.toString(mArgv));
            startOpenVPNThreadArgs(mArgv);
            Log.i(TAG, "OpenVPN process exited (read loop ended)");
        } catch (Exception e) {
            VpnStatus.logException("Starting OpenVPN Thread", e);
            Log.e(TAG, "OpenVPNThread Got exception: " + e.toString(), e);
        } finally {
            int exitvalue = 0;
            try {
                if (mProcess != null) {
                    exitvalue = mProcess.waitFor();
                    Log.i(TAG, "OpenVPN process waitFor() completed, exit value: " + exitvalue);
                } else {
                    Log.w(TAG, "OpenVPN process is null, cannot get exit value");
                }
            } catch (IllegalThreadStateException ite) {
                VpnStatus.logError("Illegal Thread state: " + ite.getLocalizedMessage());
                Log.e(TAG, "IllegalThreadStateException: " + ite.getMessage(), ite);
            } catch (InterruptedException ie) {
                VpnStatus.logError("InterruptedException: " + ie.getLocalizedMessage());
                Log.e(TAG, "InterruptedException: " + ie.getMessage(), ie);
            }
            if (exitvalue != 0) {
                String errorMsg = "Process exited with exit value " + exitvalue;
                VpnStatus.logError(errorMsg);
                Log.e(TAG, errorMsg);
            } else {
                Log.d(TAG, "Process exited with exit value 0 (normal exit)");
            }
    ...
    private void startOpenVPNThreadArgs(String[] argv) {
        ...
        try {
            Log.d(TAG, "Starting process with command: " + argvlist);
            Log.d(TAG, "LD_LIBRARY_PATH=" + lbpath);
            Log.d(TAG, "TMPDIR=" + mTmpDir);
            
            mProcess = pb.start();
            Log.d(TAG, "Process started successfully, PID: " + (mProcess != null ? "unknown" : "null"));
            ...
            mOutputStream = out;
            mStreamFuture.run();
            Log.d(TAG, "Started reading from OpenVPN process output");

            while (true) {
                String logline = br.readLine();
                if (logline == null) {
                    Log.d(TAG, "OpenVPN process output stream closed (logline == null)");
                    return;
                }

                // Log all output lines for debugging
                Log.d(TAG, "OpenVPN output: " + logline);
            ...
            String errorMsg = "Error reading from output of OpenVPN process: " + e.getMessage();
            VpnStatus.logException("Error reading from output of OpenVPN process", e);
            Log.e(TAG, errorMsg, e);
            mStreamFuture.cancel(true);
            stopProcess();
        } catch (Exception e) {
            String errorMsg = "Unexpected error in startOpenVPNThreadArgs: " + e.getMessage();
            VpnStatus.logException("Unexpected error in startOpenVPNThreadArgs", e);
            Log.e(TAG, errorMsg, e);
            mStreamFuture.cancel(true);
            stopProcess();
        }
